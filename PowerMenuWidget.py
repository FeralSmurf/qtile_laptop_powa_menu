...
class PowerMenuWidget(base._TextBox):
    ...
    def run_power_menu(self):
        try:
            ...
            subprocess.run(power_menu_script, shell=True, check=True, text=True, stderr=subprocess.PIPE)
        except subprocess.CalledProcessError as e:
            # Check if the error was due to the script being terminated by the user (e.g., pressing Escape)
            if e.returncode == 1:  # Assuming return code 1 indicates cancellation or similar non-error termination
